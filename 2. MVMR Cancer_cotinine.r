...
# [cot] and cigarettes per day [CPD]) on lung cancer.
...
################################################################################
##### Extract exposure data for MR of COT and lung cancer#####
################################################################################
...
##### conditionally independent SNPs are available in the lung cancer dataset #####
...
write.csv(COT_dat_mr_5e6, "COT_dat_mr_5e6.csv",
  row.names = FALSE
)
write.csv(
  COT_dat,
...
setwd("")
memory.limit(size = 80000)
outcome_dat_never_COT <- read_outcome_data(
  "Never.txt",
  snps = COT_dat_mr$SNP,
  sep = ",",
  snp_col = "rs_number",
...
outcome_dat_ever_COT <- read_outcome_data(
  "Ever.txt",
  snps = COT_dat_mr$SNP,
  sep = ",",
  snp_col = "rs_number",
...
##### Replace rs114612145 with rs77107237 to reflect RSID change #####
outcome_dat_never_COT[outcome_dat_never_COT == "rs114612145"] <- "rs77107237"
outcome_dat_ever_COT[outcome_dat_ever_COT == "rs114612145"] <- "rs77107237"

setwd("")
memory.limit(size = 80000)
outcome_dat_never_COT_5e6 <- read_outcome_data(
  "Never.txt",
...
outcome_dat_ever_COT_5e6 <- read_outcome_data(
  "Ever.txt",
  snps = COT_dat_mr_5e6$SNP,
  sep = ",",
  snp_col = "rs_number",
...
##### Replace rs114612145 with rs77107237 to reflect RSID change #####
outcome_dat_never_COT_5e6[outcome_dat_never_COT_5e6 == "rs114612145"] <- "rs77107237"
outcome_dat_ever_COT_5e6[outcome_dat_ever_COT_5e6 == "rs114612145"] <- "rs77107237"
...
outcome_dat_never_COT$beta.outcome <- as.numeric(
  as.character(outcome_dat_never_COT$beta.outcome)
)
outcome_dat_ever_COT$beta.outcome <- as.numeric(
  as.character(outcome_dat_ever_COT$beta.outcome)
)
outcome_dat_never_COT_5e6$beta.outcome <- as.numeric(
  as.character(outcome_dat_never_COT_5e6$beta.outcome)
)
outcome_dat_ever_COT_5e6$beta.outcome <- as.numeric(
  as.character(outcome_dat_ever_COT_5e6$beta.outcome)
)
...
outcome_dat_never_COT["beta.outcome"] <- log(
  outcome_dat_never_COT["beta.outcome"]
)
outcome_dat_ever_COT["beta.outcome"] <- log(
  outcome_dat_ever_COT["beta.outcome"]
)
outcome_dat_never_COT_5e6["beta.outcome"] <- log(
  outcome_dat_never_COT_5e6["beta.outcome"]
)
outcome_dat_ever_COT_5e6["beta.outcome"] <- log(
  outcome_dat_ever_COT_5e6["beta.outcome"]
)
...
# Checked using
# tail <- 2
# se <- abs(outcome_dat_never_COT$beta.outcome[1]/ 
# qnorm(outcome_dat_never_COT$pval.outcome[1]/tail))
# outcome_dat_never_COT$se[1]

################################################################################
##### Harmonising #####
################################################################################

dat_COT_never <- harmonise_data(
  COT_dat_mr,
  outcome_dat_never_COT,
  action = 2
)
dat_COT_ever <- harmonise_data(
  COT_dat_mr,
  outcome_dat_ever_COT,
  action = 2
)
dat_COT_never_5e6 <- harmonise_data(
  COT_dat_mr_5e6,
  outcome_dat_never_COT_5e6,
  action = 2
)
dat_COT_ever_5e6 <- harmonise_data(
  COT_dat_mr_5e6,
  outcome_dat_ever_COT_5e6,
  action = 2
)
...
proxy_needed1 <- data.frame(setdiff(COT_SNPlist$SNP, dat_COT_never$SNP))
proxy_needed2 <- data.frame(setdiff(COT_SNPlist$SNP, dat_COT_ever$SNP))
proxy_needed3 <- data.frame(setdiff(COT_SNPlist_5e6$SNP, dat_COT_never_5e6$SNP))
proxy_needed4 <- data.frame(setdiff(COT_SNPlist_5e6$SNP, dat_COT_ever_5e6$SNP))
proxy_needed5 <- data.frame(setdiff(CPD_SNPlist$SNP, dat_cpd_never$SNP))
proxy_needed6 <- data.frame(setdiff(CPD_SNPlist$SNP, dat_cpd_ever$SNP))

################################################################################
##### Run to find proxies #####
################################################################################

source("proxy_search_loop_CancerMVMR_COT.R")
...
for (i in 1:length(dat_COT_never$beta.exposure)) {
  if (dat_COT_never$beta.exposure[i] < 0) {
    dat_COT_never$beta.outcome[i] <- -1 * dat_COT_never$beta.outcome[i]
  }
  if (dat_COT_never$beta.exposure[i] < 0) {
    dat_COT_never$beta.exposure[i] <- -1 * dat_COT_never$beta.exposure[i]
  }
}
for (i in 1:length(dat_COT_ever$beta.exposure)) {
  if (dat_COT_ever$beta.exposure[i] < 0) {
    dat_COT_ever$beta.outcome[i] <- -1 * dat_COT_ever$beta.outcome[i]
  }
  if (dat_COT_ever$beta.exposure[i] < 0) {
    dat_COT_ever$beta.exposure[i] <- -1 * dat_COT_ever$beta.exposure[i]
  }
}
for (i in 1:length(dat_COT_never_5e6$beta.exposure)) {
  if (dat_COT_never_5e6$beta.exposure[i] < 0) {
    dat_COT_never_5e6$beta.outcome[i] <- -1 * dat_COT_never_5e6$beta.outcome[i]
  }
  if (dat_COT_never_5e6$beta.exposure[i] < 0) {
    dat_COT_never_5e6$beta.exposure[i] <- -1 * dat_COT_never_5e6$beta.exposure[i]
  }
}
for (i in 1:length(dat_COT_ever_5e6$beta.exposure)) {
  if (dat_COT_ever_5e6$beta.exposure[i] < 0) {
    dat_COT_ever_5e6$beta.outcome[i] <- -1 * dat_COT_ever_5e6$beta.outcome[i]
  }
  if (dat_COT_ever_5e6$beta.exposure[i] < 0) {
    dat_COT_ever_5e6$beta.exposure[i] <- -1 * dat_COT_ever_5e6$beta.exposure[i]
  }
}
...
write.csv(
  dat_COT_never_5e6,
  "dat_COT_never_5e6.csv",
  row.names = FALSE
)
write.csv(
  dat_COT_ever_5e6,
  "dat_COT_ever_5e6.csv",
  row.names = FALSE
)
write.csv(
  dat_cpd_never,
...
result_COT_never <- generate_odds_ratios(result_COT_never)

dat_COT_never <- subset(dat_COT_never, mr_keep)
mr_n_COT <- TwoSampleMR::mr_ivw(
  dat_COT_never$beta.exposure,
  dat_COT_never$beta.outcome,
  dat_COT_never$se.exposure,
  dat_COT_never$se.outcome,
  parameters = default_parameters()
)
ptr_n_COT <- data.frame(mr_n_COT["Q"])
egger_n_COT <- mr_egger_regression(
  dat_COT_never$beta.exposure,
  dat_COT_never$beta.outcome,
  dat_COT_never$se.exposure,
  dat_COT_never$se.outcome,
...
ptr_n_COT[2, 1] <- egger_n_COT["Q"]
F <- dat_COT_never$beta.exposure^2 / dat_COT_never$se.exposure^2
mF <- mean(F)
ptr_n_COT[3, 1] <- mF
...
result_COT_ever <- generate_odds_ratios(result_COT_ever)

dat_COT_ever <- subset(dat_COT_ever, mr_keep)
mr_e_COT <- TwoSampleMR::mr_ivw(
  dat_COT_ever$beta.exposure,
  dat_COT_ever$beta.outcome,
  dat_COT_ever$se.exposure,
  dat_COT_ever$se.outcome,
  parameters = default_parameters()
)
ptr_e_COT <- data.frame(mr_e_COT["Q"])
egger_e_COT <- mr_egger_regression(
  dat_COT_ever$beta.exposure,
  dat_COT_ever$beta.outcome,
  dat_COT_ever$se.exposure,
  dat_COT_ever$se.outcome,
...
ptr_e_COT[2, 1] <- egger_e_COT["Q"]
F <- dat_COT_ever$beta.exposure^2 / dat_COT_ever$se.exposure^2
mF <- mean(F)
ptr_e_COT[3, 1] <- mF

# COT never(5e6)
result_COT_never_5e6 <- mr(
  dat_COT_never_5e6,
  parameters = default_parameters(),
  method_list = subset(mr_method_list(), use_by_default)$obj
)
result_COT_never_5e6 <- generate_odds_ratios(result_COT_never_5e6)

dat_COT_never_5e6 <- subset(dat_COT_never_5e6, mr_keep)
mr_n_COT_5e6 <- TwoSampleMR::mr_ivw(
  dat_COT_never_5e6$beta.exposure,
  dat_COT_never_5e6$beta.outcome,
  dat_COT_never_5e6$se.exposure,
  dat_COT_never_5e6$se.outcome,
  parameters = default_parameters()
)
ptr_n_COT_5e6 <- data.frame(mr_n_COT_5e6["Q"])
egger_n_COT_5e6 <- mr_egger_regression(
  dat_COT_never_5e6$beta.exposure,
  dat_COT_never_5e6$beta.outcome,
  dat_COT_never_5e6$se.exposure,
  dat_COT_never_5e6$se.outcome,
...
ptr_n_COT_5e6[2, 1] <- egger_n_COT_5e6["Q"]
F <- dat_COT_never_5e6$beta.exposure^2 / dat_COT_never_5e6$se.exposure^2
mF <- mean(F)
ptr_n_COT_5e6[3, 1] <- mF

# COT ever (5e6)
result_COT_ever_5e6 <- mr(
  dat_COT_ever_5e6,
  parameters = default_parameters(),
  method_list = subset(mr_method_list(), use_by_default)$obj
)
result_COT_ever_5e6 <- generate_odds_ratios(result_COT_ever_5e6)

dat_COT_ever_5e6 <- subset(dat_COT_ever_5e6, mr_keep)
mr_e_COT_5e6 <- TwoSampleMR::mr_ivw(
  dat_COT_ever_5e6$beta.exposure,
  dat_COT_ever_5e6$beta.outcome,
  dat_COT_ever_5e6$se.exposure,
  dat_COT_ever_5e6$se.outcome,
  parameters = default_parameters()
)
ptr_e_COT_5e6 <- data.frame(mr_e_COT_5e6["Q"])
egger_e_COT_5e6 <- mr_egger_regression(
  dat_COT_ever_5e6$beta.exposure,
  dat_COT_ever_5e6$beta.outcome,
  dat_COT_ever_5e6$se.exposure,
  dat_COT_ever_5e6$se.outcome,
...
ptr_e_COT_5e6[2, 1] <- egger_e_COT_5e6["Q"]
F <- dat_COT_ever_5e6$beta.exposure^2 / dat_COT_ever_5e6$se.exposure^2
mF <- mean(F)
ptr_e_COT_5e6[3, 1] <- mF
...
egger_n_cpd <- mr_egger_regression(
  ...
  dat_cpd_never$beta.outcome,
  dat_cpd_never$se.exposure,
  dat_cpd_never$se.outcome, parameters
)
...
ISQ[1, 1] <- Isq(dat_COT_never$beta.exposure, dat_COT_never$se.exposure)

ISQ[2, 1] <- Isq(dat_COT_ever$beta.exposure, dat_COT_ever$se.exposure)

ISQ[3, 1] <- Isq(dat_COT_never_5e6$beta.exposure, dat_COT_never_5e6$se.exposure)

ISQ[4, 1] <- Isq(dat_COT_ever_5e6$beta.exposure, dat_COT_ever_5e6$se.exposure)

ISQ[5, 1] <- Isq(dat_cpd_never$beta.exposure, dat_cpd_never$se.exposure)

ISQ[6, 1] <- Isq(dat_cpd_ever$beta.exposure, dat_cpd_ever$se.exposure)
...
forest_COT_never <- mr_forest(
  mr_input(
    bx = dat_COT_never$beta.exposure,
    bxse = dat_COT_never$se.exposure,
    by = dat_COT_never$beta.outcome,
    byse = dat_COT_never$se.outcome
  ),
  methods = c("ivw", "wmedian", "egger"), snp_estimates = FALSE
)
forest_COT_ever <- mr_forest(
  mr_input(
    bx = dat_COT_ever$beta.exposure,
    bxse = dat_COT_ever$se.exposure,
    by = dat_COT_ever$beta.outcome,
    byse = dat_COT_ever$se.outcome
  ),
  methods = c("ivw", "wmedian", "egger"), snp_estimates = FALSE
)
...
forest_COT_never_5e6 <- mr_forest(
  mr_input(
    bx = dat_COT_never_5e6$beta.exposure,
    bxse = dat_COT_never_5e6$se.exposure,
    by = dat_COT_never_5e6$beta.outcome,
    byse = dat_COT_never_5e6$se.outcome
  ),
  methods = c("ivw", "wmedian", "egger"), snp_estimates = FALSE
)
forest_COT_ever_5e6 <- mr_forest(
  mr_input(
    bx = dat_COT_ever_5e6$beta.exposure,
    bxse = dat_COT_ever_5e6$se.exposure,
    by = dat_COT_ever_5e6$beta.outcome,
    byse = dat_COT_ever_5e6$se.outcome
  ),
  methods = c("ivw", "wmedian", "egger"), snp_estimates = FALSE
)
...
COT_5e6_instr <- COT_dat_mr_5e6[, c(
  "SNP",
  "effect_allele.exposure",
  "se.exposure",
  "pval.exposure",
...
INSTR[INSTR == "rs114612145"] <- "rs77107237"
...
COT_dat_mvmr_1 <- subset(COT_dat_mvmr, select = c(
  "SNP",
  "effect_allele.exposure",
...
COT_dat_mvmr_5e6_1 <- subset(COT_dat_mvmr_5e6, select = c(
  "SNP",
  "effect_allele.exposure",
  "other_allele.exposure",
  "beta.exposure",
...
proxy_needed7 <- data.frame(
  exposures[exposures$SNP %in% n_occur$Var1[n_occur$Freq < 2], ]
)
proxy_needed7 <- data.frame(proxy_needed7[1])
proxy_needed8 <- data.frame(
  exposures_5e6[exposures_5e6$SNP %in% n_occur_5e6$Var1[n_occur_5e6$Freq < 2], ]
)
...
##### Change all p-values for COT to 1e-200 for clumping so that 0 are #####
##### dropped.                                                         ##### 
...
exposures <- exposures %>%
  mutate(pval.exposure = if_else(
    exposures$SNP %in% COT_instr$SNP, 1e-201, pval.exposure
  ))

exposures_5e6 <- exposures_5e6 %>%
  mutate(pval.exposure = if_else(
    exposures_5e6$SNP %in% COT_5e6_instr$SNP, 1e-201, pval.exposure
  ))
...
CPD_H <- subset(COT_CPD, id.exposure == "2", select = c(
  SNP,
  exposure,
  id.exposure,
  effect_allele.exposure,
...
CPD_H_5e6 <- subset(COT_CPD_5e6, id.exposure == "2", select = c(
  SNP,
  exposure,
  id.exposure,
  effect_allele.exposure,
...
COT_H <- subset(COT_CPD, id.outcome == "1", select = c(
  SNP,
  outcome,
  id.outcome,
...
COT_H_5e6 <- subset(COT_CPD_5e6, id.outcome == "1", select = c(
  SNP,
  outcome,
  id.outcome,
  effect_allele.outcome,
...
Exposures_H[Exposures_H == "rs114612145"] <- "rs77107237"
Exposures_H_5e6[Exposures_H_5e6 == "rs114612145"] <- "rs77107237"

outcome_dat_ever[outcome_dat_ever == "rs114612145"] <- "rs77107237"
outcome_dat_ever_5e6[outcome_dat_ever_5e6 == "rs114612145"] <- "rs77107237"
outcome_dat_never[outcome_dat_never == "rs114612145"] <- "rs77107237"
outcome_dat_never_5e6[outcome_dat_never_5e6 == "rs114612145"] <- "rs77107237"
...
outcome_dat_never_5e6$beta.outcome <- as.numeric(as.character(
  outcome_dat_never_5e6$beta.outcome
))
outcome_dat_ever_5e6$beta.outcome <- as.numeric(as.character(
...
mvdat_never1_5e6 <- harmonise_data(Exposures_H_5e6, outcome_dat_never_5e6)
...
proxy_needed9 <- data.frame(setdiff(COT_H$SNP, outcome_dat_never$SNP))
proxy_needed10 <- data.frame(setdiff(COT_H$SNP, outcome_dat_ever$SNP))
proxy_needed11 <- data.frame(setdiff(COT_H_5e6$SNP, outcome_dat_never_5e6$SNP))
proxy_needed12 <- data.frame(setdiff(COT_H_5e6$SNP, outcome_dat_ever_5e6$SNP))

source("proxy_search_loop_CancerMVMR2_COT.R")

################################################################################
##### Re-merge the data #####
################################################################################

Exposures_H_never <- merge(COT_never, CPD_never, all = TRUE)
Exposures_H_never["Phenotype"] <- NA
Exposures_H_never$Phenotype[Exposures_H_never$id.exposure == 1] <- "COT"
Exposures_H_never$Phenotype[Exposures_H_never$id.exposure == 2] <- "CPD"
str(Exposures_H_never)

Exposures_H_never_5e6 <- merge(COT_never_5e6, CPD_never_5e6, all = TRUE)
Exposures_H_never_5e6["Phenotype"] <- NA
Exposures_H_never_5e6$Phenotype[Exposures_H_never_5e6$id.exposure == 1] <- "COT"
Exposures_H_never_5e6$Phenotype[Exposures_H_never_5e6$id.exposure == 2] <- "CPD"
str(Exposures_H_never_5e6)

Exposures_H_ever <- merge(COT_ever, CPD_ever, all = TRUE)
Exposures_H_ever["Phenotype"] <- NA
Exposures_H_ever$Phenotype[Exposures_H_ever$id.exposure == 1] <- "COT"
Exposures_H_ever$Phenotype[Exposures_H_ever$id.exposure == 2] <- "CPD"
str(Exposures_H_ever)

Exposures_H_ever_5e6 <- merge(COT_ever_5e6, CPD_ever_5e6, all = TRUE)
Exposures_H_ever_5e6["Phenotype"] <- NA
Exposures_H_ever_5e6$Phenotype[Exposures_H_ever_5e6$id.exposure == 1] <- "COT"
...
Exposures_H_never[Exposures_H_never == "rs114612145"] <- "rs77107237"
Exposures_H_never_5e6[Exposures_H_never_5e6 == "rs114612145"] <- "rs77107237"
Exposures_H_ever[Exposures_H_ever == "rs114612145"] <- "rs77107237"
Exposures_H_ever_5e6[Exposures_H_ever_5e6 == "rs114612145"] <- "rs77107237"
...
##### Convert odds ratios to log odds #####
################################################################################
outcome_dat_never$beta.outcome <- as.numeric(as.character(
  outcome_dat_never$beta.outcome
))
outcome_dat_ever$beta.outcome <- as.numeric(as.character(
...
write.csv(
  mvdat_never1_5e6,
  "mvdat_never_cot5e6.csv",
  row.names = FALSE
)
write.csv(
  mvdat_ever1_5e6,
  "mvdat_ever_cot5e6.csv",
  row.names = FALSE
)
...
(bY ~ bX1 + bX2 - 1, weights = bYse^-2))$coef[1, 2] /
...
# qhet_mvmr is used to adjust for covariance but at present, 
# CIs take substantial time to calculate
...
simexegger_COTn5e6 <- data.frame(simexegger_COTn5e6)
simexegger_COTe5e6 <- data.frame(simexegger_COTe5e6)
...
OR <- as.numeric(c(
  result_COT_never_5e6[3, "or"], mod_n_or_5e6[1, 7],
  simexegger_COTn5e6[1, "OR_weighted"], mod_ME_n_or_5e6[2, 7],
  result_cpd_never[3, "or"], mod_n_or_5e6[2, 7],
  result_cpd_never[1, "or"], mod_ME_n_2_or_5e6[3, 7],
  result_COT_ever_5e6[3, "or"], mod_e_or_5e6[1, 7],
  simexegger_COTe5e6[1, "OR_weighted"], mod_ME_e_or_5e6[2, 7],
  result_cpd_ever[3, "or"], mod_e_or_5e6[2, 7],
  result_cpd_ever[1, "or"], mod_ME_e_2_or_5e6[3, 7]
))

LCI <- as.numeric(c(
  result_COT_never_5e6[3, "or_lci95"], mod_n_or_5e6[1, 8],
  simexegger_COTn5e6[1, "lCIOR_weighted"], mod_ME_n_or_5e6[2, 8],
  result_cpd_never[3, "or_lci95"], mod_n_or_5e6[2, 8],
  result_cpd_never[1, "or_lci95"], mod_ME_n_2_or_5e6[3, 8],
  result_COT_ever_5e6[3, "or_lci95"], mod_e_or_5e6[1, 8],
  simexegger_COTe5e6[1, "lCIOR_weighted"], mod_ME_e_or_5e6[2, 8],
  result_cpd_ever[3, "or_lci95"], mod_e_or_5e6[2, 8],
  result_cpd_ever[1, "or_lci95"], mod_ME_e_2_or_5e6[3, 8]
))

UCI <- as.numeric(c(
  result_COT_never_5e6[3, "or_uci95"], mod_n_or_5e6[1, 9],
  simexegger_COTn5e6[1, "uCIOR_weighted"], mod_ME_n_or_5e6[2, 9],
  result_cpd_never[3, "or_uci95"], mod_n_or_5e6[2, 9],
  result_cpd_never[1, "or_uci95"], mod_ME_n_2_or_5e6[3, 9],
  result_COT_ever_5e6[3, "or_uci95"], mod_e_or_5e6[1, 9],
  simexegger_COTe5e6[1, "uCIOR_weighted"], mod_ME_e_or_5e6[2, 9],
  result_cpd_ever[3, "or_uci95"], mod_e_or_5e6[2, 9],
  result_cpd_ever[1, "or_uci95"], mod_ME_e_2_or_5e6[3, 9]
))

p <- as.numeric(c(
  result_COT_never_5e6[3, "pval"], mod_n_or_5e6[1, 4],
  simexegger_COTn5e6[1, "p_weighted"], mod_ME_n_or_5e6[2, 4],
  result_cpd_never[3, "pval"], mod_n_or_5e6[2, 4],
  result_cpd_never[1, "pval"], mod_ME_n_2_or_5e6[3, 4],
  result_COT_ever_5e6[3, "pval"], mod_e_or_5e6[1, 4],
  simexegger_COTe5e6[1, "p_weighted"], mod_ME_e_or_5e6[2, 4],
...
Q <- c(
  ptr_n_COT_5e6[1, 1], ptr_n_5e6[["Qstat"]],
  ptr_n_COT_5e6[2, 1], ptr_n_5e6[["Qstat"]],
  ptr_n_cpd[1, 1], ptr_n_5e6[["Qstat"]],
  ptr_n_cpd[2, 1], ptr_n_5e6[["Qstat"]],
  ptr_e_COT_5e6[1, 1], ptr_e_5e6[["Qstat"]],
  ptr_e_COT_5e6[2, 1], ptr_e_5e6[["Qstat"]],
  ptr_e_cpd[1, 1], ptr_e_5e6[["Qstat"]],
  ptr_e_cpd[2, 1], ptr_e_5e6[["Qstat"]]
)

EggerI <- c(
  ".", ".", egger_n_COT_5e6[["b_i"]], mod_ME_n_or_5e6[1, 7],
  ".", ".", egger_n_cpd[["b_i"]], mod_ME_n_2_or_5e6[1, 7],
  ".", ".", egger_e_COT_5e6[["b_i"]], mod_ME_e_or_5e6[1, 7],
  ".", ".", egger_e_cpd[["b_i"]], mod_ME_e_2_or_5e6[1, 7]
)

EggerIp <- c(
  ".", ".", egger_n_COT_5e6[["pval_i"]], mod_ME_n_or_5e6[1, 4],
  ".", ".", egger_n_cpd[["pval_i"]], mod_ME_n_2_or_5e6[1, 4],
  ".", ".", egger_e_COT_5e6[["pval_i"]], mod_ME_e_or_5e6[1, 4],
  ".", ".", egger_e_cpd[["pval_i"]], mod_ME_e_2_or_5e6[1, 4]
)

F_stat <- c(
  ptr_n_COT_5e6[3, 1], Fstat_n_5e6[1, 1],
  ptr_n_COT_5e6[3, 1], Fstat_n_5e6[1, 1],
  ptr_n_cpd[3, 1], Fstat_n_5e6[1, 2],
  ptr_n_cpd[3, 1], Fstat_n_5e6[1, 2],
  ptr_e_COT_5e6[3, 1], Fstat_e_5e6[1, 1],
  ptr_e_COT_5e6[3, 1], Fstat_e_5e6[1, 1],
...
write.csv(
  all_results, 
  "Results_COT_CPD_lung.csv", 
  row.names = FALSE
  )

MVMRproxies_cot5e6_n <- subset(SNPs.proxies11, target %in% COT_SNPlist_5e6$SNP)
MVMRproxies_cpd_n <- subset(SNPs.proxies11, target %in% CPD_SNPlist$SNP)
MVMRproxies_cot5e6_e <- subset(SNPs.proxies12, target %in% COT_SNPlist_5e6$SNP)
MVMRproxies_cpd_e <- subset(SNPs.proxies12, target %in% CPD_SNPlist$SNP)

nSNPs_COTn_5e6 <- length(intersect(
  COT_SNPlist_5e6$SNP, mvdat_never1_5e6$SNP
)) + length(intersect(
  SNPs.proxies8b$RS_Number, mvdat_never1_5e6$SNP
)) + length(intersect(MVMRproxies_cot5e6_n$RS_Number, mvdat_never1_5e6$SNP))
nSNPs_CPDn <- length(intersect(
  CPD_SNPlist$SNP, mvdat_never1_5e6$SNP
)) + length(intersect(
  SNPs.proxies8a$RS_Number, mvdat_never1_5e6$SNP
)) + length(intersect(
  MVMRproxies_cpd_n$RS_Number, mvdat_never1_5e6$SNP
))
nSNPs_COTe_5e6 <- length(
  intersect(COT_SNPlist_5e6$SNP, mvdat_ever1_5e6$SNP)
) + length(intersect(
  SNPs.proxies8b$RS_Number, mvdat_ever1_5e6$SNP
)) + length(intersect(MVMRproxies_cot5e6_e$RS_Number, mvdat_ever1_5e6$SNP))
nSNPs_CPDe <- length(intersect(
  CPD_SNPlist$SNP, mvdat_ever1_5e6$SNP
)) + length(intersect(
  SNPs.proxies8a$RS_Number, mvdat_ever1_5e6$SNP
)) + length(intersect(MVMRproxies_cpd_e$RS_Number, mvdat_ever1_5e6$SNP))

nSNPs_COTn_mr <- mr_n_COT_5e6$nsnp
nSNPs_CPDn_mr <- mr_n_cpd$nsnp
nSNPs_COTe_mr <- mr_e_COT_5e6$nsnp
...
tabletext_ever <- cbind(
  ...
  c(
    "N SNPs", nSNPs_COTe_mr, nSNPs_COTe_5e6, nSNPs_COTe_mr, nSNPs_COTe_5e6,
    nSNPs_CPDe_mr, nSNPs_CPDe, nSNPs_CPDe_mr, nSNPs_CPDe
  ),
  c(
    "Method", "MR IVW", "MVMR IVW", "MR-Egger (SIMEX)", "MVMR-Egger",
    "MR IVW", "MVMR IVW", "MR-Egger", "MVMR-Egger"
  ),
...
pdf("MVMR_ever_cot.pdf", width = 15, height = 15)
...
tabletext_never <- cbind(
  c("Exposure", "Cotinine", NA, NA, NA, "Smoking Heaviness", NA, NA, NA),
  c(
    "N SNPs", nSNPs_COTn_mr, nSNPs_COTn_5e6, nSNPs_COTn_mr, nSNPs_COTn_5e6,
...
###### Create graphs and write to file #####
pdf.options(reset = TRUE, onefile = FALSE)
pdf("MVMR_never_cot.pdf", width = 15, height = 15)
