# Script to run MR Egger SIMEX. Created by Jasmine Khouja 28/10/2022.

simexegger<-c()
simexegger_COT<-list()
simexegger_COT_5e6<-list()
...
BetaXG <- harmonise_mr_dfs_cot[[outcomes[i]]][[samples[j]]]$beta.exposure
BetaYG <- harmonise_mr_dfs_cot[[outcomes[i]]][[samples[j]]]$beta.outcome
seBetaXG <- harmonise_mr_dfs_cot[[outcomes[i]]][[samples[j]]]$se.exposure
seBetaYG <- harmonise_mr_dfs_cot[[outcomes[i]]][[samples[j]]]$se.outcome
...
output<-cbind(
  ...
  lcior2,
  ucior2)
simexegger_COT[[outcomes[i]]][[samples[j]]] <-rbind(simexegger, output)
colnames(simexegger_COT[[outcomes[i]]][[samples[j]]]) <- c(
  "Exposure", 
  "beta_weighted", 
  "lowerCI_weighted", 
  "upperCI_weighted", 
...
for (i in 1:length(outcomes)) {
  for (j in 1:length(samples)) {
    BetaXG <- harmonise_mr_dfs_cot_5e6[[outcomes[i]]][[samples[j]]]$beta.exposure
    BetaYG <- harmonise_mr_dfs_cot_5e6[[outcomes[i]]][[samples[j]]]$beta.outcome
    seBetaXG <- harmonise_mr_dfs_cot_5e6[[outcomes[i]]][[samples[j]]]$se.exposure
    seBetaYG <- harmonise_mr_dfs_cot_5e6[[outcomes[i]]][[samples[j]]]$se.outcome
...
output<-cbind(
  ...
  lcior2, 
  ucior2)
simexegger_COT_5e6[[outcomes[i]]][[samples[j]]] <-rbind(simexegger, output)
colnames(simexegger_COT_5e6[[outcomes[i]]][[samples[j]]]) <- c(
  "Exposure", 
  "beta_weighted",
  "lowerCI_weighted", 
  "upperCI_weighted", 
